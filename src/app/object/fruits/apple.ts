...
export class Apple extends Phaser.GameObjects.Image {
    ...
    constructor(scene: Phaser.Scene, x, y, handContainer: HandContainer, talkingBubbleText: Phaser.GameObjects.Text, firstFruit) {
        ...
        // slice
        let slice = this.scene.add.image(this.x, this.y, "apple_slice").setDepth(12).setScale(0).setAlpha(0).setOrigin(1, .5);
        //slice tween 
    ...
    private DiceTheFruits(): void {
        ...
        // dice step 1
        let pear_3 = this.scene.add.image(this.x, this.y, "Apple_3").setScale(0.15).setDepth(12);
        let pear_4 = this.scene.add.image(this.x, 330, "Apple_4").setScale(0.15).setDepth(12);
    ...
    private addDicedContainer(): void {
        ...
        for (let i = 0; i < 50; i++) {
            let dicedImg = this.scene.add.image(Phaser.Math.FloatBetween(-100, 100), Phaser.Math.FloatBetween(0, 100), "Diced_Apple_and_Pear_Small").setDepth(12).setScale(0.15);;
    ...
    private MashTheFruit(): void {
        ...
        const mashSound = this.scene.sound.add('Mash_sound');
        mashSound.play();

        Diced_Pear.play("Pear_Mash_Anim");
        // hand tween
        this.scene.tweens.add({
            targets: this.handContainer,
            y: this.y - 60,
