import { alphaTween, hideObject, PlaySound, showText, StopSound, tweenToPosition } from "../../scene/utils";
import { HandContainer } from "../handContainer";

export class Pear extends Phaser.GameObjects.Image {
    ...
    constructor(scene: Phaser.Scene, x, y, handContainer: HandContainer, talkingBubbleText: Phaser.GameObjects.Text, firstFruit) {
        ...
        this.on('pointerdown', () => {
            ...
            // slice
            let slice = this.scene.add.image(this.x, this.y, "slice").setDepth(12).setScale(0).setAlpha(0).setOrigin(1, .5);
            //slice tween 
    ...
    private DiceStep_2() {
        let pear_4 = this.scene.add.image(this.x, this.y + 100, "Pear_4").setScale(0.15).setDepth(12);
    ...
    private DiceStep_3(pear_4: Phaser.GameObjects.Image) {
        this.scene.tweens.add({
            ...
            onComplete: () => {
                ...
                alphaTween(this.scene, pear_4, 0, 200, 1000);
                this.Simmer();
            }
        });
    }
    ...
    // simmer step 1
    private SimmerStep_1(diced_Pear: Phaser.GameObjects.Image, soucepan: Phaser.GameObjects.Image): void {
        // dice pear tween
        this.scene.tweens.add({
            targets: diced_Pear,
            y: this.y + 60,
